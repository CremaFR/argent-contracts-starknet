...
@pytest.fixture
async def plugin_default_validate(get_starknet):
    starknet = get_starknet
    plugin_session = await deploy(starknet, "contracts/DefaultSigner.cairo")
    return plugin_session

@pytest.fixture
async def account_factory(get_starknet, plugin_default_validate):
    starknet = get_starknet
    SignerPlugin = plugin_default_validate
    #SignerPlugin = await deploy(starknet, "contracts/DefaultSigner.cairo")
    ...
    await account.initialize(SignerPlugin.contract_address, [signer.public_key, guardian.public_key]).invoke()
    return account
...
@pytest.mark.asyncio
async def test_call_dapp_no_guardian(get_starknet, dapp_factory, plugin_default_validate):
    starknet = get_starknet
    ...
    await account_no_guardian.initialize(SignerPlugin.contract_address, [signer.public_key]).invoke()
    dapp = dapp_factory
    ...
    await sender.send_transaction([
        (SignerPlugin.contract_address, 'update_plugin', [get_selector_from_name('change_signer'), new_signer.public_key])
    ], [signer])
    ...
    await assert_revert(
        sender.send_transaction([
            (SignerPlugin.contract_address, 'update_plugin', [get_selector_from_name('trigger_escape_guardian')])
        ], [new_signer])
    )
    ...
    await sender.send_transaction([
        (SignerPlugin.contract_address, 'update_plugin', [get_selector_from_name('change_guardian'), new_guardian.public_key])
    ], [new_signer])
